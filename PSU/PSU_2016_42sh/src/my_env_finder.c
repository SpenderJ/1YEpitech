...
** Last update Sun May 21 17:14:35 2017 blaise carnevillier
*/
...
char	**my_path_finder(char **env, char *str, char separator)
{
  ...
  while (env[i] != NULL)
    {
      if (my_strncmp(str, env[i], l) == SUCCESS)
	{
...
char	**my_env_adder(char **env, char *strtoadd, char *content, int ov)
{
  int	i;
  char	**newenv;
  int	a;
